...
> 14 August 2020:
    ...
    resample and smooth S_adv -> done in:
...
#           label=r'S$_{adv}$ ' + '(rmsd = %0.2f)'% rms_adv)
        ...
        ''' Only maps, adapted for the PAPER (first Figure 1)!!! '''
        ...
        # cond_nan_ori = ~np.isnan(sss_rsp.flatten()[cond_nan_smap])
        
        # ccfo = np.corrcoef(s_smap_region[cond_nan_smap][cond_nan_ori], 
        #             sss_rsp.flatten()[cond_nan_smap][cond_nan_ori])    
        
        # print('Correlation coefficient original...', ccfo)
        # print('')        
        ...
        # plt.subplot(121)
        # plt.plot(s_smap_region[cond_nan_smap][cond_nan_ori], 
        #          sss_rsp.flatten()[cond_nan_smap][cond_nan_ori], '.')
        # plt.xlabel('S$_{SMAP}$', fontsize=fs)
        # plt.ylabel('S$_{adv}$ resampled', fontsize=fs)
        # plt.axis('image')
        
        # plt.subplot(122)
        # plt.plot(s_smap_region[cond_nan_smap], 
        #          sss_rsp_s.flatten()[cond_nan_smap], '.')
        # plt.xlabel('S$_{SMAP}$', fontsize=fs)
        # plt.ylabel('S$_{adv}$ resampled and smoothed', fontsize=fs)        
        # plt.axis('image')
        
        # plt.tight_layout()
        
        # plt.savefig(dirfigppr + 'sim_Oleander_back_alt_BD_' + filename[-22:-14] + filename[-14:-7] + '_correlations.png', dpi=200)
